...
static int64_t ImGui_ImplAndroid_GetMonotonicTimeMs()
{
    struct timespec ts;
...
void ImGui_ImplAndroid_NewFrame()
{
    ...
    {
        static int64_t s_last_invalid_size_log_ms = 0;
        static int s_suppressed_invalid_size_logs = 0;
        const int64_t now_ms = ImGui_ImplAndroid_GetMonotonicTimeMs();
        if (s_last_invalid_size_log_ms == 0 || now_ms - s_last_invalid_size_log_ms >= 5000)
        {
            if (s_suppressed_invalid_size_logs > 0)
            {
                __android_log_print(ANDROID_LOG_WARN, g_LogTag,
                                    "ImGui_ImplAndroid_NewFrame skipped: invalid window size %d x %d, suppressed=%d",
                                    window_width, window_height, s_suppressed_invalid_size_logs);
            }
            else
            {
                __android_log_print(ANDROID_LOG_WARN, g_LogTag,
                                    "ImGui_ImplAndroid_NewFrame skipped: invalid window size %d x %d",
                                    window_width, window_height);
            }
            s_last_invalid_size_log_ms = now_ms;
            s_suppressed_invalid_size_logs = 0;
        }
        else
        {
            ++s_suppressed_invalid_size_logs;
        }
        return;
    }
